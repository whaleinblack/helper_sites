...
function WeatherGlyph({summary}:{summary:string}){
  ...
  if(/多云|晴时多云|partly|few clouds|scattered/i.test(summary))return <svg className="weather-glyph weather-partly" viewBox="0 0 24 24" aria-label={summary}><circle className="weather-sun" cx="8.5" cy="8.5" r="3"/><path className="weather-sun-rays" d="M8.5 2.5V4m0 9v1.5M2.5 8.5H4m9 0h1.5M4.3 4.3l1.1 1.1m6.2 6.2 1.1 1.1m0-8.4-1.1 1.1"/><path className="weather-cloud" d="M7 18a3.5 3.5 0 1 1 1.2-6.8A5 5 0 0 1 17.7 13 2.6 2.6 0 0 1 16 18H7Z"/></svg>;
      ...
      <section className="phase-body" aria-live="polite">
        {phase===1&&<div className={`phase-view ${phaseDirection}`}>
          ...
          <div className="quick-filters">
            ...
            <label className="month-filter"><span>月份</span><select value={selectedMonth} onChange={(event)=>setSelectedMonth(Number(event.target.value))} aria-label="选择登山指数月份">{monthNames.map((name,index)=><option value={index} key={name}>{name}</option>)}</select></label>
          ...
          <div className="catalog-tools"><span>{rehabMode?'优先体力度 1–2':'全部难度'} · {monthNames[selectedMonth]}</span><select value={sortKey} onChange={(event)=>setSortKey(event.target.value as SortKey)} aria-label="排序方式"><option value="recommended">月份登山指数</option><option value="distance">直线距离</option><option value="drive">驾车时间</option><option value="transit">公交时间</option><option value="elevation">最高海拔</option><option value="course-min">最低定数</option><option value="course-max">最高定数</option><option value="popularity">热门度</option></select></div>
          <div className="catalog-list">
            {filteredAreas.map((area,index)=>{const m=areaMetrics(area.id);const expanded=expandedAreaId===area.id;const active=selectedAreaId===area.id;const cardWeather=weatherByArea[area.id]??(weatherEndpoint?undefined:unconfiguredWeather);const cardLoading=!cardWeather;const monthScore=area.monthScores[selectedMonth];const grade=recommendationGrade(monthScore);const photo=mountainImages[area.id as keyof typeof mountainImages];const photoPath=`${normalizedBasePath}${photo.src}`;return <article id={`area-card-${area.id}`} key={area.id} style={{'--area-photo':`url("${photoPath}")`} as CSSProperties} className={`area-card ${active?'active':''}`} onMouseEnter={()=>setHoveredAreaId(area.id)} onMouseLeave={()=>setHoveredAreaId(null)}>
              <button type="button" className="area-select" onClick={()=>chooseArea(area.id)}>
                <span className="area-order">{String(index+1).padStart(2,'0')}</span><span className="area-name"><b>{area.name}<small className="area-prefectures">{area.prefectures.join(' / ')}</small></b><em>{m.peakCount} 峰 · 最高 {m.highest.toLocaleString()}m</em></span>
                <span className="area-course"><b className={courseTone(m.maxCourse)}>{rangeLabel(area.id)}</b><small>コース定数</small></span><span className="area-score month-insight-anchor"><b className={`grade-${grade.toLowerCase()}`}>{grade}</b><small>{monthNames[selectedMonth]}登山指数</small><MonthInsightPopover area={area} month={selectedMonth}/></span>
              </button>
